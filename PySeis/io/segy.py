...
def read_traces(_file, nt):
    #this needs work - very inefficient. test block read.
...
def read(_file):
    ...
    assert np.any(np.diff(bheader['hns'][:])) == False #check ns is constant
    assert np.any(np.diff(bheader['hdt'][:])) == False #check ns is constant
    ns = bh['hns'][0]
    dt = bh['cols'][0]
